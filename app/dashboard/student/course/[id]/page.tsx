...
export default async function StudentCoursePage({ params }: { params: { id: string } }) {
    ...
    const totalAssignments = course.modules.reduce((sum, m) => sum + m.assignments.filter(a => !a.title.startsWith('Material:')).length, 0)
    const completedAssignments = course.modules.reduce(
        (sum, m) => sum + m.assignments.filter(a => !a.title.startsWith('Material:') && a.submissions.length > 0).length, 0
    )
        ...
        <div className="max-w-[1024px] mx-auto space-y-[32px] p-[24px]">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-[24px] border-b border-[#E5E7EB] pb-[32px]">
                    ...
                    <div className="flex flex-col items-start md:items-end gap-[8px] min-w-max">
                        ...
                        <div className="text-[14px] text-[#015A86] font-medium mt-[4px]">
                            {completedAssignments}/{totalAssignments} Tasks completed
                        </div>
                        {course.materialPdfUrl && (
                            <a 
                                href={course.materialPdfUrl} 
                                target="_blank" 
                                className="mt-[12px] inline-flex items-center gap-[8px] bg-[#015A86] text-white px-[16px] py-[8px] rounded-[8px] text-[14px] font-semibold hover:bg-[#0B2E3F] transition-colors"
                            >
            ...
            <div className="space-y-[24px]">
                {course.modules.map((module, moduleIndex) => {
                    const materials = module.assignments.filter(a => a.title.startsWith('Material:'))
                    const tasks = module.assignments.filter(a => !a.title.startsWith('Material:'))
                            ...
                            <CardHeader className="bg-[#F5F8FA] border-b border-[#E5E7EB] p-[20px]">
                                <div className="flex items-center gap-[12px]">
                                    ...
                                    <span className="text-[12px] text-[#0B2E3F] opacity-60 font-medium ml-auto">
                                        {tasks.filter(a => a.submissions.length > 0).length}/{tasks.length} done
                                    </span>
                                </div>
                            </CardHeader>
                            <CardContent className="p-[20px]">
                                <Tabs defaultValue="materials" className="w-full">
                                    <TabsList className="grid w-full grid-cols-2 mb-4">
                                        <TabsTrigger value="materials">Class Material</TabsTrigger>
                                        <TabsTrigger value="assignments">Assignments</TabsTrigger>
                                    </TabsList>
                                    
                                    <TabsContent value="materials" className="space-y-[12px]">
                                        {materials.map((assignment) => (
                                            <div key={assignment.id} className="flex items-center justify-between p-4 bg-white border border-[#E5E7EB] rounded-lg">
                                                <div className="flex items-center gap-3">
                                                    <FileText className="h-5 w-5 text-[#FD8B0A]" />
                                                    <span className="font-medium text-[#0B2E3F]">{assignment.title.replace('Material: ', '')}</span>
                                                </div>
                                                {assignment.fileUrl && (
                                                    <a 
                                                        href={assignment.fileUrl} 
                                                        target="_blank" 
                                                        className="px-4 py-2 text-sm font-bold text-white bg-[#015A86] rounded-md hover:bg-[#0B2E3F] transition-colors flex items-center gap-2"
                                                    >
                                        ...
                                        {materials.length === 0 && (
                                            <div className="text-center py-8 text-muted-foreground text-sm italic">
                                                No class materials uploaded for this module.
                                            </div>
                                        )}
                                    </TabsContent>

                                    <TabsContent value="assignments" className="space-y-[16px]">
                                                    ...
                                                    <div className="flex flex-col sm:flex-row justify-between gap-[12px]">
                                                        <div className="space-y-[4px]">
                                                            <div className="flex items-center gap-[8px]">
                                                                ...
                                                                <h4 className="text-[16px] font-semibold text-[#015A86]">
                                                                    {assignment.title.startsWith('Task: ') ? assignment.title.replace('Task: ', '') : assignment.title}
                                                                </h4>
                                                            </div>
                                                        ...
                                                        <div className="flex flex-col items-end gap-[8px]">
                                                            ...
                                                            {assignment.fileUrl && (
                                                                <a 
                                                                    href={assignment.fileUrl} 
                                                                    target="_blank" 
                                                                    className="flex items-center gap-[6px] px-[12px] py-[6px] rounded-[6px] text-[13px] font-bold bg-[#FD8B0A] text-white hover:bg-[#0B2E3F] transition-all"
                                                                >
